...
interface LocationDao {
    ...
    @Query("SELECT * FROM item_location WHERE (name LIKE  '%' || :searchName || '%' OR :searchName = '') " +
            "AND (type LIKE :type OR :type = '') AND (dimension LIKE :dimension OR :dimension ='') " +
            "LIMIT :limit OFFSET :offset")
    suspend fun getAllLocationPage(offset: Int, limit: Int, searchName: String, type: String, dimension: String): List<LocationDbModel>
